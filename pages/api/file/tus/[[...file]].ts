...
export default async function handler(
  ...
  if (!isS3UploadsEnabled || !tusServer) {
    // S3 uploads can be disabled on Blob-only deployments, so short-circuit here.
    ...
    return res
      .status(503)
      .json({ message: "S3/TUS uploads are disabled on this deployment" });
  }
